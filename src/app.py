...
@solara.component
def PieChart(model):
    update_counter.get()  # 触发 Solara 自动更新
    ...
    data = df.iloc[-1]  # 取最新一帧数据
    labels = ["Active", "Quiet", "Arrested", "Audience Quiet"]
    sizes = [data["active"], data["quiet"], data["arrested"], data["audience_quiet"]]
    colors = ["#FE6100", "#648FFF", "#808080", "#33f561"]

    # 确保数据非零，否则 matplotlib 会报错
    if sum(sizes) == 0:
