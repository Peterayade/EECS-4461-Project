...
class EpsteinCivilViolence(mesa.Model):
    ...
    def __init__(
        ...
        model_reporters = {
            "active": lambda m: sum(1 for a in m.agents_by_type[Citizen] if a.state == CitizenState.ACTIVE),
            "quiet": lambda m: sum(1 for a in m.agents_by_type[Citizen] if a.state == CitizenState.QUIET),
            "arrested": lambda m: sum(1 for a in m.agents_by_type[Citizen] if a.state == CitizenState.ARRESTED),
        ...
        for cell in self.grid.all_cells:
            ...
            if klass == Cop:
                cop = Cop(self, vision=cop_vision, max_jail_term=max_jail_term,iq=modbot_iq)
                cop.move_to(cell)
            elif klass == Citizen:
                citizen = Citizen(
                    self,
                    ...
                    iq=spambot_iq,
                    identity=random.choices([0, 1], weights=[70, 30])[0]
                )
                if (citizen.identity==1):
                    citizen.state=CitizenState.AUDIENCE_QUIET
